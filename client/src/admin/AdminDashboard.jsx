...
// const ADMIN_SEEN_IDS_KEY = "cislive_admin_seen_ids_v1";

// // ========== FIXED: Proper seen IDs loading ==========
// const loadSeenIds = () => {
//   if (typeof window === "undefined")
//     return { contacts: new Set(), demos: new Set() };
//   try {
//     const raw = window.localStorage.getItem(ADMIN_SEEN_IDS_KEY);
//     if (!raw) return { contacts: new Set(), demos: new Set() };
//     const parsed = JSON.parse(raw);
//     return {
//       contacts: new Set(
//         Array.isArray(parsed.contacts) ? parsed.contacts.filter(Boolean) : []
//       ),
//       demos: new Set(
//         Array.isArray(parsed.demos) ? parsed.demos.filter(Boolean) : []
//       ),
//     };
//   } catch {
//     return { contacts: new Set(), demos: new Set() };
...
// const saveSeenIds = (contacts, demos) => {
...
// // ============================================
// // MAIN ADMIN DASHBOARD - FIXED VERSION
// // ============================================
...
//   // ========== FIXED: Separate refs for seen IDs ==========
...
//   const newNotifIdsRef = useRef(new Set());
...
//   // Initialize device ID + seen IDs on mount
//   useEffect(() => {
...
//   // ========== FIXED: Better deduplication + proper state management ==========
...
//         // ========== FIXED: First load check ==========
//         const isFirstLoad =
//           seenContactIdsRef.current.size === 0 &&
//           seenDemoIdsRef.current.size === 0;
...
//             const wasAlreadySeen = seenContactIdsRef.current.has(id);
//             if (!wasAlreadySeen) {
//               seenContactIdsRef.current.add(id);

//               // ========== FIXED: Don't create notification on first load ==========
//               if (!isFirstLoad) {
//                 const notifId = `contact_${id}`;
//                 if (!deletedNotifIdsRef.current.has(notifId)) {
//                   console.log(`🔔 NEW contact:`, c?.name);
//                   newNotifs.push({
//                     id: notifId,
//                     type: "contact",
...
//             const wasAlreadySeen = seenDemoIdsRef.current.has(id);
//             if (!wasAlreadySeen) {
...
//         saveSeenIds(seenContactIdsRef.current, seenDemoIdsRef.current);
...
//             // ========== FIXED: Proper deduplication by ID ==========
//             const existingIds = new Set(prev.map((n) => n.id));
//             const uniqueNewNotifs = burst.filter((n) => !existingIds.has(n.id));
...
//   // Fast polling: 3 seconds (increased from 2 to reduce race conditions)
//   useEffect(() => {
